...
3. ~~**Fase 1.4 — offline event queue**~~ **[done]** — cola en IndexedDB con la foto y la firma como Blobs, reenvío en orden con corte al primer fallo, identidad de cliente para idempotencia y hora real del hecho. **Falta el service worker** (que la app ABRA sin red) → siguiente.
